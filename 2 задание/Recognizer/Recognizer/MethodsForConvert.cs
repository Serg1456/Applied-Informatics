...
namespace Recognizer
{
    internal static class MethodsForConvert
    {
        ...
        //Sc-scale
        public static (double[],Bitmap) ScScale(Mat src)
        {
            ...
            int counter = 0;
            var result = new Mat(n, m, DepthType.Cv8U, 1);
            for (int i = 1; i <= n; i++)
            {
                for (int j = 1; j <= m; j++)
                {
                    var resultOfMiddleSquare = MiddleOfSquare(face, (i - 1) * l, i * l, (j - 1) * l, j * l, l);
                    resultArray[counter++] = resultOfMiddleSquare;
                    
                }
            }
            return (resultArray,face.Resize(1.0/l,Inter.Area).ToBitmap());
        }
        ...
        //DFT
        public static (double[], Bitmap) DFT(Mat src)
        {
            ...
            //plane0 = new Mat(plane0, new Rectangle(0, 0, plane0.Cols & -2, plane0.Rows & -2));
            //int cx = plane0.Cols / 2;
            //int cy = plane0.Rows / 2;
            //Mat q0 = new Mat(plane0, new Rectangle(0, 0, cx, cy));
            //Mat q1 = new Mat(plane0, new Rectangle(cx, 0, cx, cy));
            //Mat q2 = new Mat(plane0, new Rectangle(0, cy, cx, cy));
            //Mat q3 = new Mat(plane0, new Rectangle(cx, cy, cx, cy));
            //Mat tmp = new Mat();
            //q0.CopyTo(tmp);
            //q3.CopyTo(q0);
            //tmp.CopyTo(q3);
            ...
            //q2.CopyTo(q1);
            //tmp.CopyTo(q2);
            var answer = DFTForRecognizeResults(src);
            var visualResult = DFTForVisualResult(src);

            //Вычисляем изображение (понятное для человека)

            return (GetDiagonalVector(GetAreaOfImage(answer, pDft)), visualResult.ToBitmap());
        }
        private static Mat DFTForRecognizeResults(Mat src)
        {
            var optimalRows = CvInvoke.GetOptimalDFTSize(src.Rows);
            var optimalCols = CvInvoke.GetOptimalDFTSize(src.Cols);
            var s0 = new MCvScalar(0, 0, 0);
        ...
        private static Mat DFTForVisualResult(Mat src)
        {
            //Находим оптимальные размеры для реализации DFT
            var optimalRows = CvInvoke.GetOptimalDFTSize(src.Rows);
            var optimalCols = CvInvoke.GetOptimalDFTSize(src.Cols);
        ...
        private static double[,] GetAreaOfImage(Mat img, int p)
        {
            ...
            var startPosX = img.Rows / 2 - p;
            var startPoxY = img.Cols / 2 - p;
        ...
        //DCT
        public static (double[],Bitmap) DCT(Mat src)
        {
            CvInvoke.CvtColor(src, src, ColorConversion.Bgr2Gray);
            ...
            var reslut = new Mat();
            CvInvoke.Normalize(resultOfConvert, reslut, 0, 255, NormType.MinMax, DepthType.Cv8U);
            var watch = reslut.ToImage<Gray, byte>();
            return (GetDiagonalVector(GetLeftSquareOfImage(resultOfConvert, pDct)),watch.ToBitmap());
        }
        //Histogram
