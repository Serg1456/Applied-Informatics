...
{
    internal class _2DPCAEigenFaceCascade
    {
        ...
        public _2DPCAEigenFaceCascade()
        {

        }
        public void Run(List<Person> train, int numComponents)
        {
            numComponentsRows = numComponents;
            numComponentsCols = numComponents;
            etalon = train;
            CalcMean(train);
            CalcDiff(train);
            CalcCovarianceRows();
        ...
        private void CalcColsCovariance()
        {
            int K = rowsProjection.Length;
            Mat sample = rowsProjection[0].Data;
            covarianceCols = new Mat(sample.Cols, sample.Cols, sample.Depth, sample.NumberOfChannels);
            for (int i = 0; i < K; i++)
            {
                Mat currData = rowsProjection[i].Data;
        ...
        private void CalcRowsProjection(List<Person> train)
        {
            int K = diffs.Length;
            rowsProjection = new Person[K];
            for (int i = 0; i < K; i++)
            {
                Person originalPerson = train[i];
                Mat diffData = diffs[i];
                Mat projection = Mul(eigenFacesRows, diffData);
                rowsProjection[i] = new Person(originalPerson.Id, originalPerson.Label, projection);
            }
        }
        ...
        public RecognitionResult ClassifyTestImages(List<Person> test)
        {
            ...
            for (int i = 0; i < countOfTestImages; i++)
            {
                ...
                if (detectedLabel == person.Label)
                {
                    countOfRigtRecognition++; ;
                    recognitionResult[i] = countOfRigtRecognition / countOfAllRecognition * 100 ;
                }
                else
                {
                    recognitionResult[i] = countOfRigtRecognition / countOfAllRecognition * 100;
        ...
        private ReconstructionResult GetReconstructionResult()
        {
            ...
            for (int i = 0; i < numberOfParamsReduction; i++)
            {
                ...
                Mat first = Mul(rowReverse, newProjection);
                Mat second = Mul(first, colReverse);
                CvInvoke.Add(second, mean, second);
                Mat newImage = new Mat();
                //CvInvoke.Add(second, mean, second);
                //Конвертируем новое изображение в правильный формат:
                CvInvoke.Normalize(second, newImage, 0, 255, NormType.MinMax, DepthType.Cv8U);
